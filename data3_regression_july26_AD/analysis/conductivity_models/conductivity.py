"""
VENDORED, UNMODIFIED, from the Lilonfe/Estrada/Singh/Ouimet/Phillip/Dowling
soft-sensor manuscript:

    "Soft Sensors Enable Real-Time Ion Concentration Measurements"
    Lilonfe, S.; Estrada, L.; Singh, P.; Ouimet, J. A.; Phillip, W. A.;
    Dowling, A. W. Journal of Membrane Science, submission MEMSCI-S-26-01544-2
    (2026). Code repo: github.com/dowlinglab/salts-conductivity-modelling
    (local copy: /Users/adowling/DowlingLab/Membranes/salts-conductivity-modelling/).

Copied verbatim from that repo's `conductivity.py` on 2026-07-06 for
docs/prompts/conductivity_model_calibration.md (DATA3 first-principles
conductivity calibration). Do NOT edit the model math here -- see
analysis/conductivity_models/calibration_curves.py for all DATA3-specific
usage (parameters, ranges, plotting, linearity/adjudication analysis).

- `shedlovsky(...)`: single-salt Shedlovsky model, manuscript Eqs. 4-9.
- `msa(...)`: mean spherical approximation (MSA), manuscript Eqs. 11-16;
  supports single/binary/ternary salts.
"""
