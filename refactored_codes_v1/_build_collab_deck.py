...
OUT = REPO / "refactored_codes_v1/DATA3_collaborator_meeting_2026-06-11.pptx"
...
def s_title():
    ...
    textbox(s, 0.9, 2.5, 11.5, 1.5,
            [[{"text": "Parameter Identifiability & Model Fits",
               "size": 40, "color": WHITE, "bold": True, "font": HFONT}]])
    textbox(s, 0.9, 3.55, 11.5, 0.6,
            [[{"text": "Tightened per-salt bounds  ·  contour maps of every parameter  ·  "
                       "consistent contour-derived fits", "size": 16,
               "color": RGBColor(0xCC, 0xD6, 0xE2)}]])
    textbox(s, 0.9, 5.6, 11.5, 0.9,
            [[{"text": "Keshav Kasturi Rangan", "size": 16, "color": WHITE, "bold": True}],
             [{"text": "Collaborator meeting  ·  June 11, 2026", "size": 13,
...
SCRUB = {
    "sigma": ("scrub-sigma_BvsLp.gif", "scrubbing σ  ·  basin shown in B × Lₚ"),
...
def s_anim2(sheet, stage_col, stage_tag, verdict, headline, readoff):
    ...
    verdict_chip(s, verdict, 10.45, 0.36)
    gw = 7.3
    g1 = ANIM_DIR / sheet / "scrub-sigma_BvsLp.gif"
    g2 = ANIM_DIR / sheet / "scrub-B_Lpvssigma.gif"
    textbox(s, 0.5, 1.46, 7.3, 0.26, [[{"text": "①  B × Lₚ  basin   as   σ : 0 → 1", "size": 12, "color": stage_col, "bold": True, "font": HFONT}]])
    if g1.exists():
...
def s_appendix_B():
    ...
    add_image(s, ALLPAIRS / "objcontour-x_B-y_sigma.png", 6.75, 1.95, 6.0)
    textbox(s, 0.55, 1.95 + h1 + 0.1, 6.0, 0.4, [[{"text": "B × Lₚ  (σ fixed at warm-start = 1)", "size": 12, "color": GRAY, "italic": True}]], align=PP_ALIGN.CENTER)
    textbox(s, 6.75, 1.95 + h1 + 0.1, 6.0, 0.4, [[{"text": "B × σ  (Lₚ fixed) — mass argmins at interior σ ≈ 0.32", "size": 12, "color": GRAY, "italic": True}]], align=PP_ALIGN.CENTER)
    return s
