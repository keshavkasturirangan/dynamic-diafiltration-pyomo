...
Used by `tests/test_data1_regression.py` and `tests/test_data2_regression.py`.

This is the migrated `_trajectory_compare.py` from the original
`refactored_codes_v1/tests/` scaffold, with two changes:

  1. Imports from `helpers._capture_fingerprints` (the fingerprint functions)
     and from the conftest's `REL_TOLERANCE` constant.
  2. No path manipulation here — conftest.py handles sys.path.

Compared fields (all EXACT match, no tolerance):
  - data_config scalar values  (1% relative tolerance — real numbers)
  - per-vial array lengths     (exact integer equality)
  - Pyomo Var/Constraint counts (exact)
  - Pyomo Var/Constraint names  (set equality)
"""
...
# Import the fingerprint functions from the capture script (no rewrite —
# they are the source of truth for what we capture and compare).
from _capture_references import _fingerprint_data_stru, _fingerprint_pyomo_model

# REL_TOLERANCE is defined in conftest.py at session start.
REL_TOLERANCE = 1e-2
...
def run_and_compare(lib, case, ref, data_path):
    ...
    # data_config scalars within 1% (physical values).
