...
import scipy.io as spio            # MATLAB .mat file I/O
from scipy import interpolate      # 1D interpolation for time-aligned comparisons
import matplotlib.pyplot as plt    # Plotting
from matplotlib.lines import Line2D

import idaes                       # For dynamic simulation / initialization
import time                        # Timing / logging if needed
import copy                        # Deep copies of dictionaries
import os                          # File path utilities
import json                        # Storing results as JSON
from sklearn.metrics import r2_score  # R^2 metrics if needed

# --- Pyomo / IDAES imports for modeling ---
from pyomo.environ import *        # Core Pyomo modeling components
from pyomo.dae import *            # Differential-Algebraic Equation tools
import idaes.core.util.scaling as iscale  # Scaling utilities

# --- OOP experimental data loader (for Excel / unified I/O) ---
...
def loadmat(filename):  # for fun!
    ...
    This function is called instead of using spio.loadmat directly as it cures the problem
    of not properly recovering python dictionaries from mat files. It calls the function
    _check_keys to cure all entries which are still mat-objects.

    Adapted from:
        https://stackoverflow.com/questions/7008608/scipy-io-loadmat-nested-structures-i-e-dictionaries

    Arguments:
        filename: str, full path + filename of the .mat file

    Returns:
        dictionary containing structured data (e.g., including 'data_stru')
    '''
    ...
    def _check_keys(d):
        ...
        '''
        for key in d:
            # If this entry is a MATLAB struct, convert it recursively to a dict
            if isinstance(d[key], spio.matlab.mat_struct):
                d[key] = _todict(d[key])
            # If this entry is an ndarray, it might be a cell array or struct array
    ...
    def _todict(matobj):
        ...
        '''
        d = {}
        # Each MATLAB struct has _fieldnames listing its fields
        for strg in matobj._fieldnames:
            elem = matobj.__dict__[strg]
            # Recurse if field is another struct
            if isinstance(elem, spio.matlab.mat_struct):
                d[strg] = _todict(elem)
            # Recurse into ndarrays (cell arrays / struct arrays)
    ...
    def _tolist(ndarray):
        ...
        '''
        elem_list = []
        # Iterate element-by-element through the array
        for sub_elem in ndarray:
            # If element is a MATLAB struct, convert to dict
            if isinstance(sub_elem, spio.matlab.mat_struct):
                elem_list.append(_todict(sub_elem))
            # If element is another ndarray, recurse
            elif isinstance(sub_elem, np.ndarray):
                elem_list.append(_tolist(sub_elem))
            # Convert plain ints to float for numerical consistency
    ...
    # Call scipy.io.loadmat with options that keep MATLAB structs accessible
    data = spio.loadmat(filename, struct_as_record=False, squeeze_me=True)

    # Convert any remaining MATLAB structs / cell arrays into nested dicts / lists
    return _check_keys(data)


# =====================================================================
# 0.1 Existing helper: load_experiment_file (kept for backwards-compat)
# =====================================================================
def load_experiment_file(path, measure_sheet=None, index_sheet=None, quicklook=False,
                         save_prefix=None, cumulative=False, mass_only=False, cond_only=False):
    """
    Unified experimental data loader for Excel (.xlsx/.xls/.xlsm/.xlsb) and MATLAB (.mat),
    using the OOP DataLoader from experiment_dataload_OOP_v4.

    This is analogous in spirit to `loadmat`:
    - prints which file is being loaded,
    - reads and processes the experimental data,
    - optionally generates quick-look plots,
    - returns an object that you can use for downstream analysis.

    NOTE:
        This function is retained for backward compatibility. The *new* recommended
        workflow for Excel-based experiments is:
            - load_excel_experiment(...)
            - optionally build_data_stru_from_loader(...)
        which keeps the .mat workflow completely unchanged and explicit.

    Arguments
    ---------
    path : str
        Path to the experimental data file (Excel or .mat).
    measure_sheet : str or None, optional
        Optional override for the measurement sheet name (Excel only).
    index_sheet : str or None, optional
        Optional override for the index sheet name (Excel only).
    quicklook : bool, optional
        If True, generate plots (mass + conductivity) after loading.
    save_prefix : str or None, optional
        If provided, plots are saved as PNG files with this prefix.
    cumulative : bool, optional
        If True, mass plot is cumulative across vial changes.
    mass_only : bool, optional
        If True (and cond_only=False), only plot mass vs time.
    cond_only : bool, optional
        If True (and mass_only=False), only plot conductivity vs time.

    Returns
    -------
    loader : DataLoader
        The DataLoader instance containing:
            - loader.times
            - loader.masses
            - loader.retentate_cond
            - loader.permeate_cond
            - loader.windows
        plus plotting helpers:
            - loader.plot_mass(...)
            - loader.plot_conductivity(...)
            - loader.plot_quicklook(...)
    """

    print("\nLoading experimental data file =", path, "\n")

    # Instantiate the OOP loader and load data from the given path
    loader = DataLoader(path, measure_sheet=measure_sheet, index_sheet=index_sheet)
    loader.load()

    # Optional plotting behavior (mirrors the CLI logic in experiment_dataload_OOP_v4)
    if quicklook or save_prefix:
        # If user wants only mass
        if mass_only and not cond_only:
            loader.plot_mass(save_prefix=save_prefix, cumulative=cumulative)
        # If user wants only conductivity
        elif cond_only and not mass_only:
            loader.plot_conductivity(save_prefix=save_prefix)
        # Otherwise, do the "quicklook" (mass + conductivity)
        else:
            loader.plot_quicklook(save_prefix=save_prefix, cumulative=cumulative)

    # Return the loader so caller can access .times, .masses, etc.
    return loader


# =====================================================================
# 0.2 NEW: Minimal Excel loader wrapper (NO CHANGE to .mat workflow)
# =====================================================================
def load_excel_experiment(path, measure_sheet=None, index_sheet=None):
    """
    Minimal Excel loader that wraps experiment_dataload_OOP_v4.DataLoader.

    This function is intentionally small and explicit so that:
      - The existing .mat workflow (loadmat -> data_stru -> solve_model)
        remains completely unchanged.
      - Excel experiments can be read via a single call and then optionally
        converted into a 'data_stru' compatible with the rest of this file.

    Parameters
    ----------
    path : str
        Path to an Excel file (.xlsx, .xls, .xlsm, .xlsb).
    measure_sheet : str or None
        Optional override for the measurement sheet.
    index_sheet : str or None
        Optional override for the index sheet.

    Returns
    -------
    loader : DataLoader
        Loaded DataLoader instance with:
          - loader.times          (list of per-vial time arrays)
          - loader.masses         (list of per-vial mass arrays)
          - loader.retentate_cond (list of per-vial retentate conc./cond)
          - loader.permeate_cond  (list of per-vial permeate conc./cond)
          - loader.windows        (list of windows/vials)
        and plotting helpers:
          - loader.plot_mass(...)
          - loader.plot_conductivity(...)
          - loader.plot_quicklook(...)
    """
    # Create a DataLoader instance pointing to this Excel file
    loader = DataLoader(path, measure_sheet=measure_sheet, index_sheet=index_sheet)

    # Read and process the raw Excel data into structured arrays
    loader.load()

    # Return the loader to the caller for plotting or further conversion
    return loader


# =====================================================================
# 0.3 NEW: Adapter from DataLoader -> data_stru (for model compatibility)
# =====================================================================
def build_data_stru_from_loader(loader, data_config, dataset_id=1):
    """
    Convert a DataLoader instance (Excel or other) into a `data_stru`
    dictionary compatible with ALL functions in utility_v2_KKR.py
    (e.g., solve_model, calc_FIM, inter_model, plot_sim_comparison).

    The idea:
      - DataLoader gives us per-vial time/mass/cond arrays.
      - data_config gives us experiment-level parameters (delP, Temp, etc.).
      - This function combines them into the same 'data_stru' structure that
        is normally loaded from a MATLAB .mat file via loadmat(...).

    Parameters
    ----------
    loader : DataLoader
        A fully loaded DataLoader (loader.load() already called).
        Must provide:
          - loader.times
          - loader.masses
          - loader.retentate_cond
          - loader.permeate_cond
    data_config : dict
        Dictionary with experiment-level configuration matching the
        expectations in utility_v2_KKR.py, including:
            'delP', 'Temp', 'Am', 'rho', 'ni',
            'M_F0', 'M_O', 'C_D', 'C_F0',
            'Lp0', 'B0', 'sigma0',
            'namec', 'n_v0', 'n_extra', 'n_h', 'n_A'
        The key 'n' (number of vials) will be set from the loader.
    dataset_id : int or str, optional
        Value stored in data_stru['dataset'], used mainly for naming
        output files (e.g., sim_data-dat{dataset_id}.csv).

    Returns
    -------
    data_stru : dict
        Dictionary with keys:
          - 'data_config' : dict (as above, plus 'n')
          - 'data_raw'    : list of per-vial dicts with keys
                            'time', 'mass', 'cV_avg', 'cF_exp'
          - 'dataset'     : dataset identifier
    """

    # List of configuration entries that the modeling code expects to find
    required_keys = [
        'delP', 'Temp', 'Am', 'rho', 'ni',
        'M_F0', 'M_O', 'C_D', 'C_F0',
        'Lp0', 'B0', 'sigma0',
    ...
    # Check the provided data_config has all required entries
    missing = [k for k in required_keys if k not in data_config]
    if missing:
        # If anything is missing, give a clear error so the user can fix the config
        raise KeyError(
            f"data_config is missing required keys: {missing}. "
            "These are needed by the diafiltration model in utility_v2_KKR.py."
        )

    # Number of vials/windows is taken from the DataLoader
    n_vials = len(loader.times)

    # Make a shallow copy of the configuration, so we don't mutate the original dict
    cfg = dict(data_config)

    # If 'n' is already provided, ensure it matches; otherwise we set it here
    if 'n' in cfg and cfg['n'] != n_vials:
        # This mismatch would lead to inconsistent model dimensions, so raise
        raise ValueError(
            f"data_config['n'] = {cfg['n']} does not match number of vials "
            f"from loader ({n_vials}). Please fix data_config or the input data."
        )
    # Set the number of vials in the config
    cfg['n'] = n_vials

    # Build data_raw as a list of dictionaries, one per vial
    data_raw = []
    for i in range(n_vials):
        time_i = np.asarray(loader.times[i])
        mass_i = np.asarray(loader.masses[i])
        cR_i   = np.asarray(loader.retentate_cond[i])
        cP_i   = np.asarray(loader.permeate_cond[i])

        # --- OLD (causes RecursionError) ---
        # vial_dict = {
        #     'time'   : time_i,
        #     'mass'   : mass_i,
        #     'cV_avg' : cP_i,
        ...
        # --- NEW (minimal change, mimics .mat behavior) ---
        vial_dict = {
            'time'   : time_i,           # keep as numpy array (used with np.isnan etc.)
            'mass'   : mass_i,           # keep as numpy array
            # store cV_avg as a *list*, so type(...) == list
            # and the objective uses the "time series" branch
            'cV_avg' : list(cP_i),
            'cF_exp' : cR_i,             # numpy array is fine here
        }

        data_raw.append(vial_dict)
...
# =====================================================================
# 1. Plotting: compare simulation vs experimental data
#    (original code below is unchanged)
# =====================================================================
def plot_sim_comparison(data_stru, sim_stru, stirc_mass=False, plot_pred=True, lg=False, LOUD=False):
    ...
    Arguments:
        ...
        plot_pred: boolean, if plot model predictions/simulations
...
# =====================================================================
# 2. save_model, inter_model, model_construct_inter, solve_model, etc.
#    Everything below here is the original modeling / FIM logic and
#    remains unchanged from your previous utility_v2_KKR.py.
# =====================================================================
