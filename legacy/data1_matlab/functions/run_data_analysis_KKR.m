...
clc;            % Clear the command window for better readability

% Initialize variables and settings for data processing
data_stru = struct();       % Placeholder for data structure
model_stru = struct();      % Placeholder for model structure
...
% Datasets selected
% 501.1 filtration with (w/) conductivity measurements
% 501.11 filtration without (w/o) conductivity measurements
% 511.12 diafiltration w/ conductivity measurements
% 511.11 diafiltration w/o conductivity measurements

dat = [501.1];
% dat = [501.1,501.11,511.12,511.11]; % List of datasets to process in a loop

Lp_w = 3.66;         % Pure water permeability for NF90.5 membrane 
                     % (used in contour plots commented in Xinhong's code)
mod = '201cvmv';     % default for DATA; Model identifier or configuration
                     % tag (passed to model_config)


% perform default regression
reg_method = 1;      % Regression method (1 = standard least squares)
scale_opt = 0;       % No variable scaling
