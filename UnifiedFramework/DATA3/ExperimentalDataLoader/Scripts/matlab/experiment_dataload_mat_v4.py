...
# Set True for one-off debugging when field names differ across .mat files
DEBUG = False
...
def extract_mat_variables(path: str) -> Dict[str, Any]:
    ...
    if "data_stru" in mat:
        ...
        if DEBUG:
            print("data_stru fields:", getattr(ds, "_fieldnames", None))
        ...
        # Prefer MATLAB docstring name, but accept common variants across datasets
        continuous_cF = getattr(ds, "continuous_cF", None)
        if continuous_cF is None:
            continuous_cF = getattr(ds, "conductivity_cF", None)
        if continuous_cF is None:
            continuous_cF = getattr(ds, "continuous_cf", None)  # rare
        if continuous_cF is None:
            continuous_cF = getattr(ds, "continuous", None)      # rare / legacy
