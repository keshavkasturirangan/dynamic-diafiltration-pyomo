...
The v14 loader returns an `ExperimentalData` object (plus validation issues).
This wrapper:
    1) calls the v14 loader,
...
assay tables. If you need full fidelity, use the v14 loader directly.
...
# v14 file-agnostic loader (class-field based)
from experiment_dataload_OOP_v14 import load_experiment as load_experiment_v14
from experiment_dataload_OOP_v14 import ExperimentalData
...
def load_experiment(
    ...
    if ext in {".xlsx", ".xls", ".xlsm", ".xlsb"}:
        ...
        if sheet_name is None:
            ...
            exp, validation = load_experiment_v14(path, selector=selected)
        ...
        if isinstance(sheet_name, str):
            ...
            exp, validation = load_experiment_v14(path, selector=sheet_name)
            out = _exp_to_simple_dict(exp, validation)
            if plot:
        ...
        for nm in names:
            exp, validation = load_experiment_v14(path, selector=nm)
            out = _exp_to_simple_dict(exp, validation)
            by_sheet[nm] = out
            if plot:
    ...
    # ---------------------------------------------------------------------
    if ext == ".mat":
        # v14 expects selector to be a struct key or None; None -> default "data_stru".
        exp, validation = load_experiment_v14(path, selector=None)
        out = _exp_to_simple_dict(exp, validation)
        if plot:
