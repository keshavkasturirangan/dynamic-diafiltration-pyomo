...
# --------------------------------------------------------------------------------
# Unified loader for diafiltration data that reads Excel (.xlsx/.xls/.xlsm/.xlsb)
# or MATLAB (.mat) and returns Python (numpy) structures ready for analysis/plotting.
#
# Features:
# - Excel: list sheets, choose one or multiple sheets to process
# - Robust "offset header" detection
# - Vial segmentation from a 'Vial'/'Vial Swap' column (contiguous segments)
# - Optional quicklook plots with vial boundary lines and a top axis with vial labels
# - Consistent API for both Excel and MAT
# --------------------------------------------------------------------------------
...
def load_experiment(
    ...
    save_prefix : str, optional
        If provided, save PNGs to f"{save_prefix}_<sheet>_mass.png" and "_cond.png".
        For one-sheet inputs (MAT or Excel single sheet), the <sheet> token is omitted.
    sheet_name : str | list[str] | None, default None
        Excel only:
        - None  -> auto-detect a single measurement sheet (time + mass/cond)
        - str   -> process that one sheet
        - list  -> process multiple sheets; return a dict per sheet
    ...
    """
    # ---------- Basic file checks ----------
    ext = os.path.splitext(path)[1].lower()                  # get ".xlsx" or ".mat"
    if not os.path.exists(path):                             # clear message if path is wrong
        raise FileNotFoundError(f"File not found: {path}")

    # If user asked only to list sheets, do that and exit early (Excel only)
    if list_sheets:
        if ext not in {".xlsx", ".xls", ".xlsm", ".xlsb"}:   # list_sheets doesn't apply to MAT
    ...
    # ---------- Excel path: possibly multiple sheets ----------
    if ext in {".xlsx", ".xls", ".xlsm", ".xlsb"}:
        # Normalize sheet_name into a list[str] (or None) for uniform handling
        if sheet_name is None:
            # Auto-detect ONE measurement-like sheet and process that single sheet
            xl = pd.ExcelFile(path)
            sel = _autodetect_measure_sheet(xl)              # look for columns ~ time + (mass or cond)
            if sel is None:
                raise ValueError("Could not auto-detect a measurement-like sheet.")
            result = _load_excel_one_sheet(path, sel, plot=plot,
                                           save_prefix=save_prefix)  # dict for that sheet
            return result
        else:
            # Caller specified one sheet "name" or a list of sheet names
            names: List[str] = [sheet_name] if isinstance(sheet_name, str) else list(sheet_name)
            xl = pd.ExcelFile(path)
            # Validate requested names exist in workbook
            ...
            # Process each requested sheet independently and gather results
            by_sheet: Dict[str, dict] = {}
            for nm in names:
                by_sheet[nm] = _load_excel_one_sheet(path, nm, plot=plot,
                                                     save_prefix=(f"{save_prefix}_{nm}" if save_prefix else None))
            return {"sheets": names, "by_sheet": by_sheet}

    # ---------- MATLAB path: single "sheet" semantics ----------
    if ext == ".mat":
        result = _load_mat_single(path, plot=plot, save_prefix=save_prefix)  # single dict
        return result
...
def _autodetect_measure_sheet(xl: pd.ExcelFile) -> Optional[str]:
    ...
    for sh in xl.sheet_names:
        try:
            sniff = xl.parse(sh, nrows=10)                   # light parse to check columns
        except Exception:
            continue
...
def _load_excel_one_sheet(
    ...
    """
    # Open workbook and read the sheet without header first
    xl = pd.ExcelFile(path)
    raw = xl.parse(sheet, header=None)                     # raw 2D values; header may be offset
    hdr = _detect_header_row(raw)                          # find which row is the real header
    df = raw.iloc[hdr + 1 :].copy()                        # data rows (below header)
    df.columns = raw.iloc[hdr].astype(str).str.strip().tolist()  # set detected header as column names
    df = df.reset_index(drop=True)                         # clean index 0..N-1

    # Identify relevant columns (robust to naming differences by substring match)
    ...
    # Compute vial "windows" (start,end) using a Vial/Vial Swap column if available
    windows: List[Tuple[int, int]] = []
    vcol = _first_col_like(df, "vial")
    if vcol is not None:
        v = pd.to_numeric(df[vcol], errors="coerce").fillna(-1).to_numpy()
        if np.isfinite(v).any():
            # Build segment boundaries whenever the vial marker changes
            boundaries = [0] + [i for i in range(1, len(v)) if v[i] != v[i - 1]] + [len(v)]
            # Convert boundary pairs to inclusive windows; require at least 3 rows per vial
            windows = [(int(s), int(e - 1)) for s, e in zip(boundaries[:-1], boundaries[1:]) if (e - s) >= 3]
    if not windows and len(df) > 0:
        windows = [(0, len(df) - 1)]                       # fallback: treat entire sheet as one window

    # Convert chosen columns into numeric arrays, with sensible defaults if missing
    ...
    # Slice arrays into per-vial segments; compute Δmass and filter spikes > 1.2 g
    ...
    for (s, e) in windows:
        ...
        m = np.where(m > 1.2, np.nan, m)                   # spike filter per your rule
    ...
    # Optionally plot for this sheet
    if plot:
        _plot_quicklook(
...
def _load_mat_single(
    ...
    # Try common keys; adapt these if your MAT files differ
    ...
    if time is None or mass is None:                                     # must have time & mass
        raise ValueError("MAT file missing required arrays: Time and Mass")

    # Ensure 1-D arrays and consistent length based on time; pad/trim aux signals as needed
    ...
    # Windows from vial if present, else full-length
    windows: List[Tuple[int, int]] = [(0, n - 1)]
    if vial is not None:
        v = np.array(vial).squeeze()
        # Map to numeric stream to detect changes easily
...
def _plot_quicklook(
    ...
    - Top x-axis with vial labels placed at between-window centers
    """
    # Compute vial boundary x-positions: last time of each vial except the last
    boundary_times = [t[-1] for t in times[:-1] if len(t)]
    # Compute tick positions for top axis: simple mean time of each vial window
    ...
    if any(len(m) > 0 for m in masses):
        ...
        for t, m in zip(times, masses):
            mask = ~np.isnan(m)                          # ignore NaN spikes
            if mask.any():
                ax.plot(t[mask], m[mask], color="black") # SAME color for all vials

        # Draw boundary lines where vials switch
        for bt in boundary_times:
            ax.axvline(bt, color="gray", linestyle=":", linewidth=1)

        # Top x-axis labeling vials
        ax2 = ax.twiny()
        ax2.set_xlim(ax.get_xlim())                      # sync x-lims
        ax2.set_xticks(centers)                          # one tick per vial
    ...
    if any(np.isfinite(arr).any() for arr in retentate) or any(np.isfinite(arr).any() for arr in permeate):
        ...
        # Retentate (solid black)
        for t, r in zip(times, retentate):
            if r.size and len(r) == len(t):
                ax.plot(t, r, color="black")

        # Permeate (dashed black)
        for t, p in zip(times, permeate):
            if p.size and len(p) == len(t):
                ax.plot(t, p, linestyle="--", color="black")

        # Boundary lines and top axis vial labels
        for bt in boundary_times:
            ax.axvline(bt, color="gray", linestyle=":", linewidth=1)
        ax2 = ax.twiny()
