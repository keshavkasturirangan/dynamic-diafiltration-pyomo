...
#   (1) the file-agnostic experimental data loader (MAT/XLSX -> ExperimentalData)
#   (2) the diafiltration process-model construction scaffolding (Pyomo)
# Keeping everything in one file makes it easier to trace data -> model.

# Pyomo is an OPTIONAL dependency for the data-loader.
# We import it lazily so that people can load/plot data without Pyomo installed.
try:
    # Real code will use these symbols; the loader itself does not depend on them.
...
except Exception:  # pragma: no cover (only triggers when Pyomo isn't installed)
    ConcreteModel = object  # type: ignore
    Var = Param = Constraint = ConstraintList = Set = RangeSet = NonNegativeReals = object  # type: ignore
    ContinuousSet = DerivativeVar = object  # type: ignore
    value = None  # type: ignore
    pyomo_exp = None  # type: ignore
...
def plot_experiment(exp: ExperimentalData, *, kind: str = "retentate_signal") -> None:
    ...
    if kind_norm == "signals":
        # Plot retentate in all cases; add permeate only when present.
        for v in exp.vials:
            if v.time_s is None:
                continue
            ...
            if v.retentate_signal is not None:
                plt.plot(v.time_s, v.retentate_signal, label=f"vial {v.number} ret")

            # Permeate signal (optional)
            if v.permeate_signal is not None:
                plt.plot(v.time_s, v.permeate_signal, label=f"vial {v.number} perm")
    ...
    for v in exp.vials:
        ...
        plt.plot(v.time_s, y, label=f"vial {v.number}")
...
# =============================================================================
# Diafiltration process model construction (Pyomo) — scaffold matching pseudocode_v8
# =============================================================================

# NOTE:
#   The functions below are a *direct* implementation of the structure laid out
#   in pseudocode_v8.py. They are intentionally written as:
#       - small field-only classes (options/guesses)
#       - supporting functions that attach pieces to a Pyomo model
#   so you can grow this into the full model while keeping responsibilities clear.
...
class RunMode(str, Enum):
    """Workflow modes: simulation vs estimation, with room for OED."""
...
class BForm(str, Enum):
    """Named solute-permeability parameterizations used by the legacy model."""

    SINGLE = "single"        # one scalar B for all vials
    PERVIAL = "pervial"      # one B per vial
    CONVECTION = "convection"  # convection-diffusion style alternative


# B initializers can be a scalar, a per-vial list, or a dict {vial_index: value}.
BInit = Union[float, np.ndarray, List[float], Tuple[float, ...], Dict[int, float]]
...
class ModelOptions:
    ...
    Inputs:
        mode:
            Experiment mode (DATA/Lag/Overflow). This changes the ODE structure.

        run_mode:
            SIMULATION: parameters are fixed to provided values.
            ESTIMATION: parameters are free decision variables.
            OED: reserved for future MBDoE integration.

        B_form:
            Controls how solute permeability is represented:
                - BForm.SINGLE / BForm.PERVIAL / BForm.CONVECTION (named forms)
                - or a numeric exponent (legacy polynomial/exponent dependence).
    """
...
class ParameterGuess:
    """Initial guesses / fixed values for model parameters.

    IMPORTANT:
        In our "Always Vars" design, we always declare parameters as Vars.
        In SIMULATION mode we call `.fix(value)` on those Vars after creating them.

    Inputs:
        Lp:
            Water permeability-like parameter (legacy units).
        sigma:
            Reflection coefficient [-].
        theta0:
            Numeric vector of initial guesses (kept for compatibility with legacy workflows).

        B:
            Solute permeability initializer. Shape depends on B_form:
                - SINGLE: float
                - PERVIAL: list/dict/array
                - CONVECTION: may be None (uses beta_0/beta_1 instead)

        beta_0..beta_3:
            Optional coefficients used for convection / exponent-based B forms.
...
# -----------------------------
# Physical constants (legacy units)
# -----------------------------
R_BAR_CM3_PER_UMOL_K: float = 8.314e-5  # [cm^3*bar/(umol*K)]
NU_CM2_PER_S: float = 8.927e-3          # [cm^2/s]
STIRRED_CELL_DIAMETER_CM: float = 2.2860  # [cm]
RPM_DEFAULT: float = 350.0              # [rev/min]
MH_ML: float = 0.25                     # [mL]

# Diffusivity map (extensible): component_name -> D [cm^2/s]
DIFFUSIVITY_CM2_S: Dict[str, float] = {
    "K+": 1.960e-5,
    # Add more species as you characterize them.
}


def compute_avg_velocity_cm_s(*, rpm: float = RPM_DEFAULT, diameter_cm: float = STIRRED_CELL_DIAMETER_CM) -> float:
    """Compute the legacy average velocity scale v [cm/s]."""
    # Legacy correlation uses: v = (rpm/60) * pi * b
    return (rpm / 60.0) * np.pi * diameter_cm


def select_diffusivity_cm2_s(component_names: Optional[List[str]], *, diffusivities: Dict[str, float] = DIFFUSIVITY_CM2_S) -> float:
    """Select a diffusion coefficient D [cm^2/s] based on component name(s).

    Inputs:
        component_names:
            List of species names (e.g., ["K+"]) stored in ExperimentalData.
        diffusivities:
            Mapping {name: D_cm2_s}. If a requested species is missing, we raise.

    Output:
        D_cm2_s:
            Diffusion coefficient [cm^2/s] used in the mass-transfer correlation.
    """
    if not component_names:
        raise ValueError("component_names is empty; cannot choose diffusivity.")

    # Minimal rule for now: use the first component.
    # You can generalize later (mixtures, multi-ion effective D, etc.).
    key = str(component_names[0]).strip()
    if key not in diffusivities:
        raise KeyError(f"No diffusivity available for component '{key}'. Add it to DIFFUSIVITY_CM2_S.")
    return float(diffusivities[key])


def compute_mass_transfer_k_cm_s(*, v_cm_s: float, D_cm2_s: float, nu_cm2_s: float = NU_CM2_PER_S, diameter_cm: float = STIRRED_CELL_DIAMETER_CM) -> float:
    """Compute mass-transfer coefficient k [cm/s] using the legacy stirred-cell correlation."""
    # Legacy: k = 0.23 * v^0.57 * D^0.67 / (nu^0.24 * b^0.43)
    return 0.23 * (v_cm_s ** 0.57) * (D_cm2_s ** 0.67) / ((nu_cm2_s ** 0.24) * (diameter_cm ** 0.43))
...
def build_diafiltration_model(
    ...
    *,
    ...
    """Construct and return a Pyomo ConcreteModel for diafiltration.

    Inputs:
        exp:
            ExperimentalData loaded from MAT/XLSX (file-agnostic).

        options:
            Structural modeling choices (mode, run_mode, B_form).

        guess:
            Initial values (and, in SIMULATION, the values we will fix Vars to).
    ...
    Output:
        m:
            Pyomo model instance with sets/vars/constraints attached.
    """
    if ConcreteModel is object:
        # Pyomo isn't installed; raise a clear error only when someone calls the model builder.
        raise ImportError("Pyomo is required for build_diafiltration_model(), but it is not installed in this environment.")

    # ---------------------------------------------------------------------
    # 0) Pull commonly-used experiment fields (keep them honest + explicit)
    # ---------------------------------------------------------------------
    ...
    # Defensive: these are required by the legacy equations.
    if delP_bar is None or T_K is None or Am_cm2 is None or rho_g_cm3 is None:
        raise ValueError("exp is missing one of the required fields: delP_bar, Temp_K, Am_cm2, rho_g_cm3")

    # Dialysate concentration (legacy uses constant cD)
    cD_value = exp.C_D_value
    if cD_value is None:
        raise ValueError("exp.C_D_value is required for model construction (dialysate concentration).")

    # Initial feed mass and concentration
    M_F0_g = exp.M_F0_g
    C_F0 = exp.C_F0_value
    if M_F0_g is None or C_F0 is None:
        raise ValueError("exp.M_F0_g and exp.C_F0_value are required for model construction.")

    # Overflow mass (used in Lag/Overflow modes)
    M_O_g = exp.M_O_g

    # ---------------------------------------------------------------------
    # 1) Compute correlation terms (v, D, k)
    # ---------------------------------------------------------------------
    v_cm_s = compute_avg_velocity_cm_s()  # [cm/s]
    D_cm2_s = select_diffusivity_cm2_s(exp.component_names)  # [cm^2/s]
    k_cm_s = compute_mass_transfer_k_cm_s(v_cm_s=v_cm_s, D_cm2_s=D_cm2_s)  # [cm/s]

    # ---------------------------------------------------------------------
    # 2) Time bookkeeping (per-vial bounds)
    # ---------------------------------------------------------------------
    # Minimal version: use vial time arrays as-is.
    # A later patch can replicate the full legacy ti/tf logic (and scaling factors).
    if not exp.vials:
        raise ValueError("exp.vials is empty; cannot build a dynamic model without vial segmentation.")

    # Legacy uses a delay equal to first timestamp in the first vial.
    t_delay_s = float(exp.vials[0].time_s[0]) if exp.vials[0].time_s is not None else 0.0

    # Build ti/tf lists (unscaled)
    ti_list: List[float] = []
    tf_list: List[float] = []
    for v in exp.vials:
        if v.time_s is None or len(v.time_s) == 0:
            raise ValueError(f"Vial {v.number}: missing time_s.")
        ti_list.append(float(v.time_s[0]) - t_delay_s)
        tf_list.append(float(v.time_s[-1]) - t_delay_s)

    # Scaled time domain tau ∈ [0, Tauf]
    ...
    # Number of vials (1-based like the legacy)
    N_VIAL = len(exp.vials)
    m.n_vial = RangeSet(N_VIAL)

    # Scaled continuous time
    m.tau = ContinuousSet(bounds=(0.0, Tauf))

    # Convenience dicts: map vial index -> (tf - ti)
    TF_dict = dict(zip(range(1, N_VIAL + 1), tf_list))
    TI_dict = dict(zip(range(1, N_VIAL + 1), ti_list))

    # ---------------------------------------------------------------------
    # 4) Declare parameters-as-Vars (Always Vars; SIMULATION fixes them)
    # ---------------------------------------------------------------------
    # Water permeability-like parameter
    m.Lp = Var(bounds=(0.5, 50.0), initialize=float(guess.Lp))

    # Reflection coefficient
    m.sigma = Var(bounds=(0.0, 1.0), initialize=float(guess.sigma))

    # Solute permeability parameterization depends on B_form
    if options.B_form == BForm.SINGLE:
        init_B = float(guess.B) if guess.B is not None else 1e-6
        m.B = Var(bounds=(1e-6, 30.0), initialize=init_B)
    elif options.B_form == BForm.PERVIAL:
        # Per-vial initializer: accept dict/list/scalar
        def _init_B_pervial(n: int) -> float:
            if guess.B is None:
                return 1e-6
            if isinstance(guess.B, dict):
                return float(guess.B.get(int(n), 1e-6))
            if isinstance(guess.B, (list, tuple, np.ndarray)):
                idx = int(n) - 1
                return float(guess.B[idx]) if 0 <= idx < len(guess.B) else 1e-6
            return float(guess.B)

        m.B = Var(m.n_vial, bounds=(1e-6, 30.0), initialize=_init_B_pervial)
    else:
        # CONVECTION or numeric exponent forms will be expanded in later steps.
        # We declare placeholders so the model object is structurally consistent.
        m.B = Var(m.n_vial, m.tau, bounds=(1e-6, 50.0), initialize=1e-6)
        m.beta_0 = Var(initialize=guess.beta_0 if guess.beta_0 is not None else 1.0)
        if guess.beta_1 is not None:
            m.beta_1 = Var(initialize=float(guess.beta_1))
        if guess.beta_2 is not None:
            m.beta_2 = Var(initialize=float(guess.beta_2))
        if guess.beta_3 is not None:
            m.beta_3 = Var(initialize=float(guess.beta_3))

    # SIMULATION: fix Vars to provided values
    if options.run_mode == RunMode.SIMULATION:
        m.Lp.fix(float(guess.Lp))
        m.sigma.fix(float(guess.sigma))
        if options.B_form == BForm.SINGLE:
            m.B.fix(float(guess.B) if guess.B is not None else value(m.B))
        elif options.B_form == BForm.PERVIAL:
            # Fix each per-vial B
            for n in m.n_vial:
                m.B[n].fix(value(m.B[n]))
        # For other forms, we'll fix whatever was declared as needed later.

    # ---------------------------------------------------------------------
    # 5) Declare state variables (subset of legacy; expand constraint-by-constraint)
    # ---------------------------------------------------------------------
    # Feed concentration and holdup concentration
    ...
    # Permeate vial mass and concentration proxies
    m.mV = Var(m.n_vial, m.tau, domain=NonNegativeReals, initialize=1e-6)
    m.cVmV = Var(m.n_vial, m.tau, initialize=1e-12)
    ...
    # ---------------------------------------------------------------------
    # 6) Attach a minimal subset of the legacy constraints (enough to validate wiring)
    # ---------------------------------------------------------------------
    # NOTE: These are skeletal forms; you'll extend them to match the full legacy model.

    def eqn_cV_rule(m, n, t):
        # Numerically stable alternative to cV = cVmV/mV
        return m.mV[n, t] * m.cV[n, t] == m.cVmV[n, t]

    m.eqn_cV = Constraint(m.n_vial, m.tau, rule=eqn_cV_rule)

    def ode_mV_rule(m, n, t):
        # dmV/dt = Jw * Am * rho  (scaled by vial duration)
        scale = (TF_dict[int(n)] - TI_dict[int(n)]) / Tauf
        return m.dmV[n, t] == m.Jw[n, t] * Am_cm2 * rho_g_cm3 * scale

    m.ode_mV = Constraint(m.n_vial, m.tau, rule=ode_mV_rule)

    # ---------------------------------------------------------------------
    # 7) Initial conditions (minimal; you will align with legacy IC logic next)
    # ---------------------------------------------------------------------
    m.con_boundary = ConstraintList()
    m.con_boundary.add(m.cF[1, 0.0] == float(C_F0))
    m.con_boundary.add(m.cH[1, 0.0] == 1e-6)
    m.con_boundary.add(m.mV[1, 0.0] == 1e-6)
    m.con_boundary.add(m.cVmV[1, 0.0] == 1e-12)

    # Attach a few constants to the model for convenience/debugging (optional)
    m._exp_cD_value = float(cD_value)  # type: ignore[attr-defined]
    m._k_cm_s = float(k_cm_s)          # type: ignore[attr-defined]
    m._v_cm_s = float(v_cm_s)          # type: ignore[attr-defined]
    m._D_cm2_s = float(D_cm2_s)        # type: ignore[attr-defined]
