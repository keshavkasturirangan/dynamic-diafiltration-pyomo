...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct  9 11:22:51 2025

@author: kkasturi

OOP-structured unified data loader for Excel (.xlsx/.xls/.xlsm/.xlsb) and MATLAB (.mat)
files used in diafiltration experiments.

Key ideas vs your original script:
- Clear separation of concerns via classes (sources, window strategies, run container, plotting mixin).
- Same public API after `load()`: times, masses, retentate_cond, permeate_cond, windows.
- Extensible: add new WindowStrategy or DataSource without touching the orchestrator.
- Plotting obeys your constraints: matplotlib only, one figure per chart, **no explicit colors**.

Usage
-----
from ooph_structured_data_loader import DataLoader
ldr = DataLoader(path)
ldr.load()
ldr.plot_quicklook("run1")

Public attributes after load():
- ldr.times: List[np.ndarray]
- ldr.masses: List[np.ndarray]
- ldr.retentate_cond: List[np.ndarray]
- ldr.permeate_cond: List[np.ndarray]
- ldr.windows: List[tuple[int, int]] (indices over the measurement vectors)
"""
...
@dataclass
class DataRun:
    """A full, raw measurement run (prior to windowing)."""
    time: np.ndarray
    mass: np.ndarray
    retentate_cond: np.ndarray  # may be NaN
    permeate_cond: np.ndarray   # may be NaN
    # Optional auxiliary column that encodes vial/swap IDs (numeric or categorical)
    vial_marker: Optional[np.ndarray] = None

    def __post_init__(self):
        # Ensure 1D np.float arrays and aligned lengths by padding/truncating aux arrays
        ...
        def _fit(x, fill=np.nan):
            ...
            return arr
        self.time = _fit(self.time, fill=np.nan).astype(float)
        self.mass = _fit(self.mass, fill=np.nan).astype(float)
        self.retentate_cond = _fit(self.retentate_cond, fill=np.nan).astype(float)
        self.permeate_cond = _fit(self.permeate_cond, fill=np.nan).astype(float)
...
class IndexSheetStrategy:
    """Use an Excel index sheet with Start/End or jstart/jend to define windows."""
    def infer(self, source: ExcelSource, meas_df: Optional[pd.DataFrame], run: DataRun) -> List[Tuple[int, int]]:
...
class VialMarkerStrategy:
    """Segment windows where a vial marker column changes value (Excel or MAT)."""
    def infer(self, source: ExcelSource, meas_df: Optional[pd.DataFrame], run: DataRun) -> List[Tuple[int, int]]:
        v = run.vial_marker
        if v is None or np.asarray(v).size == 0:
            return []
        v_arr = np.asarray(v).squeeze()
        # Try numeric coercion; if fails, fall back to categorical change detection
        try:
...
class SingleWindowStrategy:
    """Fallback: one window spanning the entire run."""
    def infer(self, source: ExcelSource, meas_df: Optional[pd.DataFrame], run: DataRun) -> List[Tuple[int, int]]:
        n = run.time.size
...
class QuicklookPlotMixin:
    """Provides plot_quicklook(save_prefix=None) with required plotting rules."""
    times: List[np.ndarray]
    masses: List[np.ndarray]
    retentate_cond: List[np.ndarray]
    permeate_cond: List[np.ndarray]

    def plot_quicklook(self, save_prefix: Optional[str] = None) -> None:
        boundary_times = []
        for t in self.times[:-1]:
            if len(t):
                boundary_times.append(t[-1])
        # Plot 1: mass vs time (overlay all vials) — no explicit colors, one figure
        if any(len(m) > 0 for m in self.masses):
            fig, ax = plt.subplots()
            for t, m in zip(self.times, self.masses):
                mask = ~np.isnan(m)
                if mask.any():
                    ax.plot(t[mask], m[mask])
            ax.set_xlabel("Time [s]")
            ax.set_ylabel("Mass change [g]")
            ax.set_title("Mass vs Time")
            for bt in boundary_times:
                ax.axvline(bt, linestyle=":", linewidth=1)
            ax2 = ax.twiny()
            ax2.set_xlim(ax.get_xlim())
            centers = [ti.mean() for ti in self.times if len(ti)]
        ...
        # Plot 2: conductivity vs time — single figure, default styles only
...
class DataLoader(QuicklookPlotMixin):
    """
    Facade class that selects a DataSource by file extension, applies a chain of
    WindowStrategy objects, and exposes the final per-window arrays via a simple API.
    """
    def __init__(self, path: str):
        ...
        self._meas_df: Optional[pd.DataFrame] = None  # populated for Excel only
        # Strategy order: index-sheet > vial-marker > single-window
    ...
    def load(self) -> None:
        ...
        self._run = run
        # Excel-only handle to measurement df for debugging; optional.
        if isinstance(self._source, ExcelSource):
            # Re-parse to capture the cleaned measurement sheet for inspection
            xl = pd.ExcelFile(self.path)
            measure_sheet = self._source._detect_measurement_sheet(xl)
        ...
        # slice into per-window arrays and post-process mass (delta + spike filter)
        ...
        for (s, e) in self.windows:
            ...
            m = np.where(m > 1.2, np.nan, m)  # spike filter (Δmass > 1.2 g -> NaN)
    ...
    # Optional getters for debugging/inspection
    @property
