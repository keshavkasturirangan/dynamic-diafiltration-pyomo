# `unified_codebase_runfile.py`

> **Canonical code path (source of truth):** `UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/`  
> Runner entrypoint: `UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/unified_codebase_runfile.py`  
> Core pipeline: `UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/unified_codebase_library.py`  
> Legacy root-level scripts (for example `utility.py`, `run_*.py`) are compatibility/reference paths, not primary development paths.


## Purpose

`unified_codebase_runfile.py` is the user-facing runner script.  
It now supports profile-based switching for:

- `DATA1` reproduction,
- `DATA2` reproduction,
- `DATA3` experiments (custom file/sheet workflow).

Heavy lifting remains delegated to `unified_codebase_library.py`.

## Reproduction Standard

This runner is part of the long-term replacement path for the published-paper workflows.

The intended standard is:
- DATA1 and DATA2 paper results must be reproducible from the unified codebase
- original legacy files such as `DATA1-matlab/`, `utility.py`, `DATA1_model_demo.ipynb`, `DATA2_model_demo.ipynb`, and `DATA2_visualization.ipynb` are reference implementations only
- those legacy files must not be edited in order to make paper reproduction succeed
- a plot should only be described as "generated by the unified codebase" when the underlying simulation/analysis data and the rendered figure both come from the unified workflow

In practice, this means `unified_codebase_runfile.py` plus `unified_codebase_library.py` should eventually own:
- DATA1 estimation and paper-figure recreation
- DATA2 estimation, model-variation logic, and paper-figure recreation
- workflow-managed paper styling and comparison artifacts

Temporary use of legacy outputs is acceptable for validation and gap analysis, but not as the final reproduction endpoint.

## Execution flow

1. Select `--profile DATA1|DATA2|DATA3`.
2. Resolve default file/selector/model-options from profile.
3. Apply optional overrides (`--file-path`, `--selector`, `--nfe`, flags).
4. Build `UnifiedPipelineConfigV24`.
5. Execute `run_unified_pipeline_v24`.
6. Print run summary (load status, theta labels, estimation/DoE output).

## Profiles

- `DATA1`
  - files:
    - `DATA1_matlab/data_library/data_stru-dataset501.1.mat`
    - `DATA1_matlab/data_library/data_stru-dataset501.11.mat`
    - `DATA1_matlab/data_library/data_stru-dataset511.11.mat`
    - `DATA1_matlab/data_library/data_stru-dataset511.12.mat`
  - defaults: `mode=DATA`, `b_form=single`, non-logit sigma, MAT legacy restart enabled
- `DATA2`
  - files:
    - `DATA1_matlab/data_library/data_stru-dataset270511.123.mat`
    - `DATA1_matlab/data_library/data_stru-dataset270611.123.mat`
  - defaults: `mode=Lag`, `b_form=convection`, `fix_sigma_in_estimation=True`
- `DATA3`
  - file: `UnifiedFramework/ExperimentalDataFiles/NF270_MC2.xlsx` (override recommended)
  - default selector/specs enabled for quick XLSX experiments
  - default `run_doe=True` (can be overridden with `--no-run-doe`)

## Key CLI options

- `--profile DATA1|DATA2|DATA3`
- `--file-path <path>`
- `--selector <xlsx_sheet_name>`
- `--nfe <int>`
- `--run-doe`
- `--no-run-doe`
- `--calc-cov`
- `--convert-to-concentration`
- `--plot`
- `--solver ipopt`

## Typical output

- load status and validation issues,
- theta names (unknown parameter set),
- number of labeled measurements and inputs,
- estimated objective and theta,
- covariance (if converged),
- FIM and D-optimality metric.

## Example commands

```bash
# DATA1 reproduction baseline
python UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/unified_codebase_runfile.py \
  --profile DATA1 --nfe 30

# DATA2 reproduction baseline with DoE
python UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/unified_codebase_runfile.py \
  --profile DATA2 --run-doe --nfe 30

# DATA3 experiment from custom XLSX
python UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/unified_codebase_runfile.py \
  --profile DATA3 \
  --file-path /abs/path/to/experiment.xlsx \
  --selector "SheetName" \
  --nfe 30

# Single-file override for DATA1 or DATA2
python UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/unified_codebase_runfile.py \
  --profile DATA1 \
  --file-path /abs/path/to/data_stru-dataset511.12.mat
```

## Operational guidance

- Start with `nfe=30` for stable estimation/covariance.
- Increase `nfe` only when needed for accuracy.
- Keep `solver="ipopt"` for ParmEst.
- If DoE struggles, tune NLP settings before increasing model complexity.

## Packaging/refactor notes

- This file should become a thin CLI entrypoint (`console_scripts`) in the package.
- Config values should move into a validated config object or YAML schema.
- Logging should replace `print` for reproducible runs.
