...
## Non-Negotiable Reproduction Requirement

The end-state unified codebase must be able to reproduce the published DATA1 and DATA2 paper analyses and figures without modifying the original legacy source files.

Reference-only legacy sources:
- `/Users/kkasturi/GitHub/keshav-dev-dynamic-diafiltration-pyomo/DATA1-matlab/`
...
These files define the published-paper reference behavior, but they are not the implementation target.

Required unified implementation target:
- primary scientific/model logic lives in `/Users/kkasturi/GitHub/keshav-dev-dynamic-diafiltration-pyomo/UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/unified_codebase_library.py`
- user-facing execution lives in `/Users/kkasturi/GitHub/keshav-dev-dynamic-diafiltration-pyomo/UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/unified_codebase_runfile.py`
- paper-style artifact generation must be owned by the unified workflow/artifact layer under `UnifiedFramework/DATA3/ExperimentalDataAnalysis/UnifiedCode/`

Acceptance rule:
- a figure only counts as "regenerated by the unified codebase" if both the numerical data behind it and the rendered plot are produced by the unified code path
- using legacy MAT/CSV/PNG outputs directly as the plotted source is acceptable only as a temporary validation reference, not as final unified reproduction
- differences between DATA1 and DATA2 should be expressed through one shared process-model family with activated/deactivated terms and profile settings, not through separate end-to-end code stacks

Immediate implication for current work:
- DATA1 Figure 5 and Figure 6 are not complete until their contour grids are generated from the unified model itself rather than from legacy contour CSVs
- DATA2 paper figures should follow the same standard
...
- Do not edit legacy paper-generation sources to make reproduction work. Instead, capture their behavior in unified code and document any remaining parity gaps explicitly.
