...
def plot_experiment(exp: ExperimentalData, *, kind: str = "retentate_signal") -> None:
    ...
    """
    import matplotlib.pyplot as plt  # local import keeps core loader lighter for non-plot use

    if exp.vials is None or len(exp.vials) == 0:
        raise ValueError("Experiment has no vials to plot.")

    # Map plot kind -> attribute name on VialData
    attr = {
    ...
    # Figure: one line per vial segment
    plt.figure()
    for v in exp.vials:
        if v.time_s is None:
            continue
        ...
        # NOTE: We do not force concatenation across vials; plotting each segment is honest.
        plt.plot(v.time_s, y, label=f"vial {v.number}")

    plt.xlabel("time (s)")
    plt.ylabel(kind)
    plt.title(f"{exp.filename or 'experiment'} — {kind}")
    plt.legend()
    plt.tight_layout()
    plt.show()
...
# =============================================================================
# Optional test: run one MAT and one XLSX experiment through the loader
# =============================================================================

if __name__ == "__main__":
    """
    Minimal optional test for the file-agnostic loader.
    ...
    print("\n==============================")
    print("OPTIONAL TEST: MAT loader")
    print("==============================")
    try:
    ...
    print("\n==============================")
    print("OPTIONAL TEST: XLSX loader (one sheet)")
    print("==============================")
    try:
        import pandas as pd
        xls = pd.ExcelFile(xlsx_path)
