...
class ExperimentalData:
    ...
    n_v0: Optional[int] = None
    n_extra: Optional[int] = None
    n_vial_total: Optional[int] = None
...
def load_from_mat(mat_path: Path, struct_key: Optional[str]) -> ExperimentalData:
    ...
    exp.n_v0 = cfg.get("n_v0", None)
    exp.n_extra = cfg.get("n_extra", None)
    exp.n_vial_total = cfg.get("n", None)
    ...
    exp.ni_species = cfg.get("ni", None)
...
class ModelOptions:
    ...
    beta_0_lb: float = 1.0 + 1e-6
    beta_0_ub: float = 50.0
    beta_1_lb: float = 0.0
    beta_1_ub: float = 1.0
    use_multistart_for_xlsx: bool = False
...
def _legacy_vial_gate_config(exp: ExperimentalData) -> Tuple[int, int]:
    """Return legacy vial-gating indices (n_v0, n_extra) with safe defaults."""
    n_total = max(1, len(exp.vials))
    n_v0 = 1
    n_extra = 0

    if exp.n_v0 is not None:
        try:
            n_v0 = int(exp.n_v0)
        except Exception:
            n_v0 = 1
    if exp.n_extra is not None:
        try:
            n_extra = int(exp.n_extra)
        except Exception:
            n_extra = 0

    n_v0 = max(1, min(n_v0, n_total))
    n_extra = max(0, min(n_extra, n_total))
    return n_v0, n_extra
...
def _legacy_data1_mode_cH0_from_first_vial(exp: ExperimentalData) -> float:
    ...
    cV0 = v0.cV_avg
    base = None
    if isinstance(cV0, (int, float, np.number)):
        val = float(cV0)
        if np.isfinite(val):
            base = val
    elif cV0 is not None:
        arr = np.asarray(cV0, dtype=float).reshape(-1)
        for val in arr:
            if np.isfinite(val):
                base = float(val)
                break
    if base is None:
        base = 1e-6
    return float(max(1e-12, 0.8 * base))
...
def build_guess_from_experiment_v24(
    exp: ExperimentalData,
    options: ModelOptions,
    *,
    override: Optional[ParameterGuess] = None,
) -> ParameterGuess:
    """Build a robust guess using MAT-provided theta0/Lp0/B0/sigma0 when available."""
    base = _default_guess(exp)
    if exp.theta0 is not None:
        try:
            th = np.asarray(exp.theta0, dtype=float).reshape(-1)
            if th.size >= 1 and np.isfinite(th[0]):
                base.Lp = float(th[0])
            if th.size >= 2 and np.isfinite(th[1]):
                base.B = float(th[1])
                base.beta_0 = float(th[1])
            if th.size >= 3 and np.isfinite(th[2]):
                base.sigma = float(th[2])
        except Exception:
            pass
    ...
    if str(options.b_form).lower() == BForm.CONVECTION.value:
        if base.beta_0 is None:
            if base.B is not None and base.Lp is not None and exp.delP_bar is not None and float(exp.delP_bar) != 0:
                ...
                base.beta_0 = 2.0
        if base.beta_1 is None:
            base.beta_1 = 0.5

    if override is None:
        return base

    merged = ParameterGuess(
        Lp=float(override.Lp if override.Lp is not None else base.Lp),
        sigma=float(override.sigma if override.sigma is not None else base.sigma),
        B=override.B if override.B is not None else base.B,
        beta_0=override.beta_0 if override.beta_0 is not None else base.beta_0,
        beta_1=override.beta_1 if override.beta_1 is not None else base.beta_1,
        beta_2=override.beta_2 if override.beta_2 is not None else base.beta_2,
        beta_3=override.beta_3 if override.beta_3 is not None else base.beta_3,
        S0=override.S0 if override.S0 is not None else base.S0,
        S=override.S if override.S is not None else base.S,
    )
    return merged
...
def theta_component_list(m: pyo.ConcreteModel, options: ModelOptions) -> List[pyo.ComponentData]:
    ...
    """
    def _is_estimated(comp: pyo.ComponentData) -> bool:
        """Return True when a component should be exposed as an estimated theta."""
        try:
            if hasattr(comp, "fixed"):
                return not bool(comp.fixed)
        except Exception:
            pass
        return True
    ...
    if hasattr(m, "sigma_logit"):
        if _is_estimated(m.sigma_logit):
            comps.append(m.sigma_logit)
    else:
        if _is_estimated(m.sigma):
...
def model_construct_inter_v23(
    ...
    if b_form == BForm.SINGLE.value:
        ...
        if sim_opt:
            ...
            m.beta_0 = pyo.Var(bounds=(float(options.beta_0_lb), float(options.beta_0_ub)), initialize=beta0)
            m.beta_1 = pyo.Var(bounds=(float(options.beta_1_lb), float(options.beta_1_ub)), initialize=beta1)
    ...
    if options.mode == ExperimentMode.DATA:
        def mV_linking_rule(mm, n):
            if n == mm.n_vial.last():
                return pyo.Constraint.Skip
            return mm.mV[n + 1, 0.0] == 1e-6
        m.mV_linking = pyo.Constraint(m.n_vial, rule=mV_linking_rule)

        def cVmV_linking_rule(mm, n):
            if n == mm.n_vial.last():
                return pyo.Constraint.Skip
            return mm.cVmV[n + 1, 0.0] == mm.cV[n, Tauf] * 1e-6
        m.cVmV_linking = pyo.Constraint(m.n_vial, rule=cVmV_linking_rule)
...
def _interpolate_state_at_scaled_time(
    state: pyo.Var,
    n_vial: int,
    t_scaled: float,
    tau_values: Sequence[float],
):
    """Return linear-interpolated state expression at a scaled time point."""
    if len(tau_values) == 0:
        raise ValueError("tau_values cannot be empty.")
    t = float(t_scaled)
    tau_min = float(tau_values[0])
    tau_max = float(tau_values[-1])
    if t <= tau_min:
        return state[n_vial, tau_min]
    if t >= tau_max:
        return state[n_vial, tau_max]

    arr = np.asarray(tau_values, dtype=float)
    hi = int(np.searchsorted(arr, t, side="left"))
    if hi <= 0:
        return state[n_vial, float(arr[0])]
    if hi >= len(arr):
        return state[n_vial, float(arr[-1])]
    lo = hi - 1
    t_lo = float(arr[lo])
    t_hi = float(arr[hi])
    if abs(t_hi - t_lo) <= 1e-14:
        return state[n_vial, t_lo]
    w = (t - t_lo) / (t_hi - t_lo)
    return (1.0 - w) * state[n_vial, t_lo] + w * state[n_vial, t_hi]
...
def _build_measurement_records(
    ...
    *,
    ...
    """Build measurement records as (pred_expr, observed_value, sigma)."""
    tau_vals = sorted(float(t) for t in list(m.tau))
    TI_dict, TF_dict, t_delay = _scaled_times(exp)
    n_v0, n_extra = _legacy_vial_gate_config(exp)

    records: List[Tuple[object, float, float]] = []
    for n in m.n_vial:
        n_i = int(n)
        vial = exp.vials[n_i - 1]
        t_meas = np.asarray(vial.time_s, dtype=float) - t_delay
        dur = TF_dict[n_i] - TI_dict[n_i]
        if dur <= 0:
            continue
        t_scaled = (t_meas - TI_dict[n_i]) / dur

        if vial.mass_g is not None and n_i >= n_v0:
            mass = np.asarray(vial.mass_g, dtype=float).reshape(-1)
            for tm, y in zip(t_scaled, mass):
                if np.isnan(y):
                    continue
                pred = _interpolate_state_at_scaled_time(m.mV, n_i, float(tm), tau_vals)
                records.append((pred, float(y), float(sigma_mass_g)))
        ...
        if n_i > n_extra:
            if isinstance(vial.cV_avg, (int, float, np.number)):
                y = float(vial.cV_avg)
                if not np.isnan(y) and y != 0:
                    pred = _interpolate_state_at_scaled_time(m.cV, n_i, 1.0, tau_vals)
                    sigma = float(max(sigma_cV_rel * abs(y), 1e-8))
                    records.append((pred, y, sigma))
            elif vial.cV_avg is not None:
                cv_arr = np.asarray(vial.cV_avg, dtype=float).reshape(-1)
                for tm, y in zip(t_scaled, cv_arr):
                    if np.isnan(y) or y == 0:
                        continue
                    pred = _interpolate_state_at_scaled_time(m.cV, n_i, float(tm), tau_vals)
                    sigma = float(max(sigma_cV_rel * abs(float(y)), 1e-8))
                    records.append((pred, float(y), sigma))

    return records


def _build_measurement_records_by_type(
    m: pyo.ConcreteModel,
    exp: ExperimentalData,
    *,
    sigma_mass_g: float,
    ...
    """Build typed weighted-residual expressions for legacy grouped objective."""
    tau_vals = sorted(float(t) for t in list(m.tau))
    TI_dict, TF_dict, t_delay = _scaled_times(exp)
    n_v0, n_extra = _legacy_vial_gate_config(exp)

    mass_terms: List[object] = []
    cf_terms: List[object] = []
    cv_terms: List[object] = []
        ...
        if vial.mass_g is not None and n_i >= n_v0:
            ...
            for tm, y in zip(t_scaled, mass):
                if np.isnan(y):
                    continue
                pred = _interpolate_state_at_scaled_time(m.mV, n_i, float(tm), tau_vals)
                mass_terms.append(((pred - float(y)) / sigma_mass_g) ** 2)
        ...
        if n_i > n_extra:
            if isinstance(vial.cV_avg, (int, float, np.number)):
                ...
                if not np.isnan(y) and y != 0:
                    pred = _interpolate_state_at_scaled_time(m.cV, n_i, 1.0, tau_vals)
                    sigma = float(max(sigma_cV_rel * abs(y), 1e-8))
                    cv_terms.append(((pred - y) / sigma) ** 2)
            elif vial.cV_avg is not None:
                cv_arr = np.asarray(vial.cV_avg, dtype=float).reshape(-1)
                for tm, y in zip(t_scaled, cv_arr):
                    if np.isnan(y) or y == 0:
                    ...
                    pred = _interpolate_state_at_scaled_time(m.cV, n_i, float(tm), tau_vals)
                    sigma = float(max(sigma_cV_rel * abs(float(y)), 1e-8))
                    cv_terms.append(((pred - float(y)) / sigma) ** 2)

    return {"mass": mass_terms, "cF": cf_terms, "cV": cv_terms}


def _ensure_measurement_proxy_components(
    m: pyo.ConcreteModel,
    exp: ExperimentalData,
    *,
    sigma_mass_g: float,
    ...
    """Create interpolation-consistent proxy variables for measured outputs once."""
    if hasattr(m, "_obs_idx") and hasattr(m, "_obs_pred"):
        return

    records = _build_measurement_records(
        m,
        exp,
        sigma_mass_g=sigma_mass_g,
        sigma_cV_rel=sigma_cV_rel,
        sigma_cF_rel=sigma_cF_rel,
    )
    idx = list(range(1, len(records) + 1))
    obs_vals = {i: records[i - 1][1] for i in idx}
    sigmas = {i: records[i - 1][2] for i in idx}
    pred_exprs = {i: records[i - 1][0] for i in idx}

    m._obs_idx = pyo.Set(initialize=idx, ordered=True)
    m._obs_pred = pyo.Var(m._obs_idx)
    m._obs_obs = pyo.Param(m._obs_idx, initialize=obs_vals, mutable=False)
    m._obs_sigma = pyo.Param(m._obs_idx, initialize=sigmas, mutable=False)
    m._obs_link = pyo.ConstraintList()
    for i in idx:
        m._obs_link.add(m._obs_pred[i] == pred_exprs[i])
...
def attach_weighted_least_squares_objective(
    ...
    *,
    ...
    _ensure_measurement_proxy_components(
        m,
        exp,
        sigma_mass_g=sigma_mass_g,
        sigma_cV_rel=sigma_cV_rel,
    ...
    expr = sum(
        ((m._obs_pred[i] - m._obs_obs[i]) / m._obs_sigma[i]) ** 2
        for i in m._obs_idx
    )
...
def attach_legacy_grouped_weighted_objective(
    m: pyo.ConcreteModel,
    exp: ExperimentalData,
    *,
    sigma_mass_g: float = 0.01,
    ...
    """Attach legacy utility.py grouped objective (for DATA/MAT parity fits)."""
    ...
    n_v0, n_extra = _legacy_vial_gate_config(exp)
    _ = n_v0  # kept for readability; gating already applied in builder
    collect_vial = max(1, len(exp.vials) - n_extra)

    n_mass = max(1, len(terms["mass"]))
    n_cf = max(1, len(terms["cF"]))
    expr_mass = sum(terms["mass"]) / n_mass
    expr_cv = sum(terms["cV"]) / collect_vial
    expr_cf = sum(terms["cF"]) / n_cf
    legacy_expr = 1e4 * (expr_mass + expr_cv + expr_cf)

    m.FirstStageCost = pyo.Expression(expr=0.0)
    m.SecondStageCost = pyo.Expression(expr=legacy_expr)
    m.Total_Cost_Objective = pyo.Objective(expr=m.FirstStageCost + m.SecondStageCost, sense=pyo.minimize)
...
def label_parmest_and_doe_suffixes(
    ...
    *,
    ...
    for i in m._obs_idx:
        key = m._obs_pred[i]
        y = float(pyo.value(m._obs_obs[i]))
        sigma = float(pyo.value(m._obs_sigma[i]))
        m.experiment_outputs[key] = y
        m.measurement_error[key] = sigma
...
def _estimate_parameters_ipopt_fallback(
    ...
    *,
    ...
    exp = experiments[0]
    guess_eff = build_guess_from_experiment_v24(exp, options, override=guess)
    m = model_construct_for_parmest_v24(exp, options=options, guess=guess_eff)
    ...
    if use_multistart:
        raw, _, _ = _solve_with_ipopt_restarts(
            m,
            options,
            solver_options=solver_options,
            tee=tee,
            objective_component=m._ipopt_fallback_wsse,
            theta_components=theta_component_list(m, options),
        )
    else:
...
def _solve_with_ipopt_restarts(
    m: pyo.ConcreteModel,
    options: ModelOptions,
    *,
    solver_options: Optional[Dict[str, object]],
    tee: bool,
    objective_component,
    theta_components: List[pyo.ComponentData],
) -> Tuple[object, float, Dict[str, float]]:
    """Utility-style randomized restart loop around ipopt for one model."""
    ...
    n_tries = max(1, int(options.multistart_iterations))
    theta_vars = [c for c in theta_components if isinstance(c, pyo.Var) and (not c.fixed)]
    best_obj = float("inf")
    best_theta_vals: Dict[str, float] = {}
    best_raw = None

    def _rand_in_bounds(v: pyo.Var) -> float:
        lb = pyo.value(v.lb) if v.lb is not None else None
        ub = pyo.value(v.ub) if v.ub is not None else None
        if lb is not None and ub is not None and np.isfinite(lb) and np.isfinite(ub) and ub > lb:
            return float(rng.uniform(lb, ub))
        v0 = float(pyo.value(v)) if pyo.value(v) is not None else 1.0
        span = max(1.0, abs(v0))
        return float(v0 + rng.uniform(-0.5, 0.5) * span)

    for k in range(n_tries):
        if k > 0:
            for v in theta_vars:
                try:
                    v.set_value(_rand_in_bounds(v))
                except Exception:
                    pass
        raw_k = solver.solve(m, tee=tee)
        term = getattr(getattr(raw_k, "solver", None), "termination_condition", None)
        if term is None or str(term).lower() not in {"optimal", "locallyoptimal"}:
            continue
        obj_k = float(pyo.value(objective_component))
        if obj_k < best_obj:
            best_obj = obj_k
            best_raw = raw_k
            best_theta_vals = {c.name: float(pyo.value(c)) for c in theta_components}

    if best_raw is None:
        raw = solver.solve(m, tee=tee)
        obj = float(pyo.value(objective_component))
        theta = {c.name: float(pyo.value(c)) for c in theta_components}
        return raw, obj, theta

    for c in theta_components:
        if c.name in best_theta_vals:
            c.set_value(best_theta_vals[c.name])
    return best_raw, best_obj, best_theta_vals
...
def model_construct_for_legacy_fit_v24(
    exp: ExperimentalData,
    options: ModelOptions,
    guess: Optional[ParameterGuess] = None,
) -> pyo.ConcreteModel:
...
def estimate_parameters_with_parmest_v24(
    ...
    *,
    ...
    if (
        ...
        and options.mode == ExperimentMode.DATA
        and str(options.b_form).lower() == BForm.SINGLE.value
    ):
        exp0 = experiments[0]
        guess_eff = build_guess_from_experiment_v24(exp0, options, override=guess)
        m = model_construct_for_legacy_fit_v24(exp0, options=options, guess=guess_eff)
        theta_comps = theta_component_list(m, options)
        if bool(options.use_multistart_for_mat_legacy):
            raw, objective, theta_vals = _solve_with_ipopt_restarts(
                m,
                options,
                solver_options=solver_options,
                tee=tee,
                objective_component=m.Total_Cost_Objective,
                theta_components=theta_comps,
            )
        else:
            ...
            raw = solver_obj.solve(m, tee=tee)
            objective = float(pyo.value(m.Total_Cost_Objective))
            theta_vals = {c.name: float(pyo.value(c)) for c in theta_comps}
        theta_vals = _augment_theta_with_sigma(theta_vals, options=options)
        return {
            "raw": raw,
            "objective": objective,
            ...
            "model": m,
            "warning": (
                "Used legacy grouped DATA/MAT objective path for fit parity with utility.py. "
                "ParmEst covariance is skipped in this mode."
            ),
        }
    ...
    guess_eff = guess
    if len(experiments) >= 1:
        guess_eff = build_guess_from_experiment_v24(experiments[0], options, override=guess)
    exp_list = build_experiment_list_v24(experiments, options, guess_eff)
    ...
    try:
        ...
        if (
            ...
            result = _estimate_parameters_ipopt_fallback(
                ...
                tee=tee,
                use_multistart=(
                    experiments[0].source == SourceType.XLSX
                    and bool(options.use_multistart_for_xlsx)
                ),
            )
            warning_msgs.append(
                "ParmEst theta_est failed with ipopt; used direct ipopt solve on labeled model for one experiment."
            )
            if warning_msgs:
...
@dataclass
class UnifiedPipelineConfigV24:
    """Configuration for a single unified MAT/XLSX run."""
    ...
    report_hook: Optional[Callable[[Dict[str, object]], None]] = None


def _default_model_options_for_source_v24(source: SourceType) -> ModelOptions:
    """Return default model options tuned by source type while keeping one API."""
...
def _resolve_model_options_v24(
...
def _build_unified_run_metadata_v24(
    ...
    """Build shared run metadata used by reporting and downstream scripts."""
    return {
...
def _run_unified_estimation_stage_v24(
    config: UnifiedPipelineConfigV24,
    exp: ExperimentalData,
    options: ModelOptions,
) -> Dict[str, object]:
    """Run shared estimation stage with one call path for MAT and XLSX."""
    return estimate_parameters_with_parmest_v24(
        experiments=[exp],
        options=options,
...
def _run_unified_doe_stage_v24(
    config: UnifiedPipelineConfigV24,
    exp_obj: DiafiltrationExperimentV24,
) -> Dict[str, object]:
    """Run shared DOE stage with one call path for MAT and XLSX."""
    return run_doe_with_pyomo_v24(
        exp_obj,
...
def run_unified_pipeline_v24(config: UnifiedPipelineConfigV24) -> Dict[str, object]:
    ...
    out: Dict[str, object] = {
        ...
        "exp_obj": exp_obj,
        "model": model,
        "theta_names": theta_names(model, options),
        "labeled_counts": {
            "outputs": len(model.experiment_outputs),
            "unknowns": len(model.unknown_parameters),
