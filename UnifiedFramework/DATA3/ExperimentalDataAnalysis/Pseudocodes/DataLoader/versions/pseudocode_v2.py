...
Goal:
    ...
    and store the result in explicit, readable dataclass fields (no dictionary "extras" dumping).

Design choices:
    1) Conductivity is stored honestly as a signal (not silently converted to concentration).
       A later measurement model can convert conductivity -> concentration if/when desired.
    2) Exactly two light classes are used: ExperimentalData and VialData.
    3) Validation flags issues without crashing; missing values remain None.

Notes:
    This is pseudocode: functions like read_excel_sheet(), read_mat_file(), parse_time_series_table()
    represent format-specific I/O/parsing steps you will implement with pandas/openpyxl/scipy.io later.
"""
...
class VialData:
    ...
    Core fields:
        ...
        retentate_signal:
            ...
            For MAT: whatever cF_exp represents in the struct; label honestly if known.
    ...
    Optional time-series fields (all aligned with time_s if present):
        pressure_psi, retentate_temp_C, permeate_temp_C, permeate_signal, vial_swap_flag.
    ...
    IMPORTANT typing decision:
        cV_avg is a UNION because legacy datasets may store it as:
            - scalar (float)
            - array (np.ndarray) e.g., NaN-filled time-series with one final value
        Using a precise Union keeps schema readable while preserving flexibility.
    """
...
def segment_indices_by_swap(swap_flags: np.ndarray) -> List[Tuple[int, int]]:
    """
    Converts a 0/1 swap flag array into vial segments (start,end) for Python slicing.
...
def build_vial_from_slice(
    *,
    ...
    """
    Creates and populates a VialData object by slicing time-aligned arrays [a:b].
    ...
    Inputs:
        ...
        swap_all, pressure_all, ret_temp_all, perm_temp_all: optional arrays aligned to time_all
    ...
    v.retentate_signal = ret_signal_all[a:b]  # retentate conductivity for this vial

    # Store honest labels so downstream code knows what "retentate_signal" means
...
def validate_experiment(exp: ExperimentalData) -> Tuple[bool, List[Tuple[str, str]]]:
    """
    Validates basic structural integrity and flags missing values without raising exceptions.
    ...
    # FATAL: each vial must have time and retentate signal, and lengths must match
    for v in exp.vials:
