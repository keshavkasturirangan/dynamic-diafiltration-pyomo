...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Restore results/reproduction/<run_id>/figures from archive.")
    ...
    parser.add_argument(
        "--archive-root",
        type=Path,
        default=Path("results/reproduction/_archives"),
    ...
    parser.add_argument("--overwrite", action="store_true", help="Replace existing figures/ if present.")
...
def main() -> None:
    ...
    if figures_dir.exists() and not args.overwrite:
        raise SystemExit("[restore] figures/ already exists. Re-run with --overwrite to replace.")
