...
Policy:
- Keep full artifacts (including figures/) for the newest N runs.
- Keep full artifacts for any run referenced by validation breadcrumbs/docs.
- For all other runs, prune figures/ by either:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="archive",
        help="How to prune figures/ for older runs.",
    )
    parser.add_argument(
        "--archive-root",
        type=Path,
...
def protected_runs_from_ledger(repo_root: Path, run_ids: Iterable[str]) -> Set[str]:
    ...
    ledger = repo_root / "docs/validation/continuity_ledger.csv"
